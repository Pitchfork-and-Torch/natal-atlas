...
import { NatalWheel, cssTint } from "./wheel.js?v=1.2.6";
import { buildModel, createAtlas, el, fmtPos, fmtPosHouse, fmtDelta, fmtCoord, soloLabel } from "./atlas.js?v=1.2.6";
import { castChart, parseBirthForm, rehouse, synastryHits } from "./cast.js?v=1.2.6";
import { searchCities, ensureCities, findCityByTz, TIMEZONES } from "./cities.js?v=1.2.6";
import { waitingChart } from "./ephemeris.js?v=1.2.6";
import { listCharts, saveChart, getChart, removeChart, exportVault, importVault } from "./vault.js?v=1.2.6";
import {
  natalPool,
  isoDate,
  nextSolarReturnDate,
...
import {
  ...
  hitLabel,
  jewelKind,
} from "./chronograph.js?v=1.2.6";
import { chronographLede } from "./lexicon.js?v=1.2.6";
import { downloadPlate, parseBirthHash, birthHash, chartToHashInput } from "./plate.js?v=1.2.6";
...
function fillStrip(model, dom) {
  ...
  const xs = members.map((m) => ((m.lon - sign.start) / 30) * 100);
  dom.stripGlow.style.setProperty("--from", `${Math.max(0, Math.min(...xs) - 8).toFixed(1)}%`);
  dom.stripGlow.style.setProperty("--to", `${Math.min(100, Math.max(...xs) + 8).toFixed(1)}%`);
  ...
  dom.stripMarks.replaceChildren(
    ...members.map((m, i) =>
      el("button", {
        ...
        style: `--x: ${xs[i].toFixed(2)}%; --tint: ${cssTint(m.id)}`,
...
function paint(chart, dom) {
  const run = () => {
    ...
    } else if (dom.elementDiamond) {
      dom.elementDiamond.replaceChildren();
    }
