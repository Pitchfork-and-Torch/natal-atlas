...
export function declutter(lons, minSep, iterations = 48) {
  ...
  const idx = lons.map((_, i) => i).sort((i, j) => norm(lons[i]) - norm(lons[j]));
  const pos = idx.map((i) => norm(lons[i]));
...
/** Push marks apart on a line (cluster legend). Order is preserved. Stays in [lo, hi]. */
export function spreadLinear(xs, minSep, lo = 0, hi = 100, iterations = 64) {
  const n = xs.length;
  if (n === 0) return [];
  if (n === 1) return [clamp(xs[0], lo, hi)];
  const idx = xs.map((_, i) => i).sort((i, j) => xs[i] - xs[j]);
  const pos = idx.map((i) => clamp(xs[i], lo, hi));
  const span = hi - lo;
  const need = minSep * (n - 1);
  if (need >= span - 1e-9) {
    const step = span / (n - 1);
    const out = new Array(n);
    idx.forEach((i, k) => {
  ...
  for (let k = 0; k < n - 1; k++) {
    if (pos[k + 1] < pos[k] + minSep) pos[k + 1] = pos[k] + minSep;
  }
  if (pos[n - 1] > hi) {
    pos[n - 1] = hi;
    for (let k = n - 2; k >= 0; k--) pos[k] = Math.min(pos[k], pos[k + 1] - minSep);
    pos[0] = Math.max(lo, pos[0]);
    for (let k = 1; k < n; k++) pos[k] = Math.max(pos[k], pos[k - 1] + minSep);
  }
  const out = new Array(n);
  idx.forEach((i, k) => {
...
/**
 * Wheel size that leaves stacked hero copy (title, kicker) unobscured.
 * Desktop keeps the 280 floor. Short phones may shrink to 180.
 */
export function fitWheelSize({ wrapW, innerH, stackedCopyH = 0 }) {
  const w = Math.max(0, Number(wrapW) || 0);
  const h = Math.max(0, Number(innerH) || 0);
  const copy = Math.max(0, Number(stackedCopyH) || 0);
  const chrome = copy > 0 ? 72 : 150;
  const availH = Math.max(0, h - chrome - copy);
  const raw = Math.floor(Math.min(w, availH) * 0.94);
  const floor = copy > 0 ? 180 : 280;
  if (!w) return Math.max(floor, raw);
  return Math.max(Math.min(floor, w), Math.min(raw, w));
}

/**
 * Cluster chapter marks: glow hugs the true longitudes, buttons are spread
 * so labels do not stack on a tight stellium.
 */
export function clusterLegendMarks(lons, signStart, minSep = 8) {
  const raw = lons.map((lon) =>
    clamp((((norm(lon) - norm(signStart) + 360) % 360) / 30) * 100, 0, 100),
  );
  if (!raw.length) return { raw: [], xs: [], from: 0, to: 100 };
  return {
    raw,
    xs: spreadLinear(raw, minSep, 0, 100),
    from: Math.max(0, Math.min(...raw) - 8),
    to: Math.min(100, Math.max(...raw) + 8),
  };
}
...
export class NatalWheel {
  ...
  resize() {
    ...
    if (this.opts.mini) {
      ...
      let stackedCopyH = 0;
      if (stacked) {
        const hero = wrap.closest(".hero") || document;
        const copy = hero.querySelector?.(".hero-copy");
        stackedCopyH = copy ? copy.offsetHeight || 0 : 0;
      }
      const wrapW = wrap.clientWidth || host.clientWidth || Math.floor((window.innerWidth || 0) * 0.5);
